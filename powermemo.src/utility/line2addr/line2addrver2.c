...
char *get_FuncName(char **str)
{
	...
	//if (tmp_str != NULL) printf("FuncName=%s\n", tmp_str);
	return tmp_str;
}
...
int parse_hex(char *str, unsigned long *value )
{
  ...
  *value = 0 ;
  ...
  while( ( c = *buf_ptr++) != '\0' )
    if( isxdigit( c ) ) /* check if input char is hex symbol? */
    {
      c = (char) toupper( (unsigned short)c ); /* convert char symbol to upper case */
      if( (c -= '0') > 9 ) /* 'A' ~ 'F' */
						...
						if (src_file != NULL) {
							...
							if (line_num > 0) {
								...
								if (atoi(src_line) >= line_num) {
									...
									strncpy(tmp_str, cmd, 8);
									insn.word = strtol(tmp_str, NULL, 16);
									//printf("insn.word=%x\n", insn.word);
									//printf("has delayslot:%s %d\n", tmp_str, insn_has_delayslot(insn));	
									//printf("%s:%s:%d:%s:%s\n", src_file, src_line, line_num, header, cmd);
									if (insn_has_delayslot(insn))
									{
										parse_hex(header, &addr);
										//printf("original ins:%x\n", addr);
										addr +=4;
										sprintf(header, "%x", addr);
										//printf("new ins:%s\n", header);
									}
									printf("address:%s\n", header);
									exit(EXIT_SUCCESS);
