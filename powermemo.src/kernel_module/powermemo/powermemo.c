...
struct mk_fun_data mk_fun_head;
...
#define MK_BUF_LEN			15000//256 /* only 8bits are used for fid in ioctl(), so we can have max 256 marker functions */
...
int markerfunc_entry(int pid, int fid, char *funname);
int markerfunc_exit(int pid, int fid, char *funname);
...
void mk_fun_free(void)
{
	struct list_head *listptr = NULL;
	struct mk_fun_data *entry = NULL;
	struct markerfunc_u *pmk = NULL;

	list_for_each(listptr, &mk_fun_head.list) {
		entry = list_entry(listptr, struct mk_fun_data, list);
		pmk = entry->mk;
		#ifdef DEBUG_FULL
		if (pmk) {
			printk("%s=>PID:%d FID:%d t0:%x t1:%x Fun:%s\n", __FUNCTION__, pmk->pid, pmk->fid, pmk->t_entry, pmk->t_exit, pmk->funname);
		}		
		#endif
		kfree(entry);
	}
}
void mk_fun_show(void)
{
	struct list_head *listptr;
	struct mk_fun_data *entry;
	struct markerfunc_u *pmk = NULL;
...
void mk_fun_remove(struct mk_fun_data *entry)
{
	struct markerfunc_u *pmk = NULL;

	if(entry == NULL){
		printk("%s: Invalid kprobe data entry\n", __FUNCTION__);
		return;
	}
	pmk = entry->mk;
	#ifdef DEBUG_FULL
...
struct mk_fun_data * mk_fun_add (struct markerfunc_u *pmk)
{
	struct mk_fun_data *pmkdata;

	if (!pmk)
	{
		printk("%s=> can't add data to mk list\n", __FUNCTION__);
		return NULL;
	}

	pmkdata = kmalloc(sizeof(struct mk_fun_data), GFP_KERNEL);	
	if (pmkdata) {
		pmkdata->mk = pmk;
		list_add(&pmkdata->list, &mk_fun_head.list);
		return pmkdata; 
	}
	return NULL;
}

struct mk_fun_data *mk_fun_find(int pid, int fid, char *fun)
{
    struct list_head *listptr = NULL;
    struct mk_fun_data *entry = NULL;
    struct markerfunc_u *pmk = NULL;
	...
	#ifdef DEBUG_FULL
	printk("%s=>Can't find pid:%d fid:%d fun:%s\n", __FUNCTION__, pid, fid, fun);
	#endif
	return NULL;
}
...
unsigned long gettime(void)
{
  ...
  /* calculate the time elapsed in granularity of 20 microseconds
	...
	diff =  (tv.tv_sec - sync_tv.tv_sec)* 50000 + 
			 ...
			 */	
	#else
	diff =  (tv.tv_sec - sync_tv.tv_sec)* 1000000 + 
            (tv.tv_usec - sync_tv.tv_usec); 
			/* remember that you can't use floating point numbers in kernel. If you really need,
			 * use shift operators
			 */
	#endif			
	...
	*(int *)(rcv_buf+4) = 0;
    ...
    list_for_each(listptr, &mk_fun_head.list) {
        ...
        if (entry) {
			...
			pmk = entry->mk;
			mk2 = (struct markerfunc_u *)(markerfunc_buf + 8 + nr_markerfunc_u * sizeof(struct markerfunc_u));  
			mk2->pid = pmk->pid;
			mk2->fid = pmk->fid;
			mk2->t_entry = pmk->t_entry;
			memcpy(mk2->funname, pmk->funname, 32);
			mk2->funname[31] = '\0';
			entry->mk = mk2;			
			
        	if (entry->mk)
				printk("%s=>PID:%d FID:%d t0:%x t1:%x Fun:%s\n", __FUNCTION__, entry->mk->pid, entry->mk->fid, entry->mk->t_entry, entry->mk->t_exit, entry->mk->funname);    		
			(*(int *)markerfunc_buf)++;
        }
    }
...
int process_slice_exit(int pid) 
{
   ...
   if(diff > 4290000000ul ){ //4294967295 is the max diff can be hold, it takes (4294967296/50000) seconds to overflow
...
int markerfunc_entry(int pid,int fid, char *funname) 
{
	...
	int nr_markerfunc_u; 
	unsigned long flags;
	struct mk_fun_data *pmkdata = NULL;
	struct markerfunc_u *pmk = NULL;
	...
	if (nr_markerfunc_u >= MK_BUF_LEN-10){
	...
	*/
	...
	mk->t_entry = gettime();
	mk->t_exit = 0;
	memcpy(mk->funname, funname, 32);
	mk->funname[31] = '\0';

	pmkdata = mk_fun_add(mk);
	#ifdef DEBUG_FULL
	if (pmkdata)
	{
		pmk = pmkdata->mk;
...
int markerfunc_exit(int pid,int fid, char *funname) 
{
	...
	unsigned long flags;	
	int lock_held_count;
	struct mk_fun_data *pmk_data = NULL;	
	struct markerfunc_u *pmk = NULL;
	...
	pmk_data = mk_fun_find(pid, fid, funname);

	if (pmk_data == NULL)
	{
		log_lock = LOG_UNLOCKED;
		printk("%s can't find correct entry pid:%d fid:%d %s\n", __FUNCTION__, pid, fid, funname);
		return 0;
	}
	if (diff > 4290000000ul ){ //4294967295 is the max diff can be hold, it takes (4294967296/50000) seconds to overflow
#warning THIS MAY NOT BE UNSIGNED FOR THE C STANDARD USED...
		pmk_data->mk->t_exit = 0;
		printk("\nThis time slice is longer than the module can handle\n");
#warning I SHOULD HANDLE THIS IN A BETTER WAY!
	}
	else
		pmk_data->mk->t_exit = diff; 
	...
	#endif
	log_lock = LOG_UNLOCKED;
	/* function information has been stored to kernel buffer */
	mk_fun_remove(pmk_data);
	local_irq_restore(flags);
...
{
	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)processor_buf1 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(processor_buf1 + 4) = 0;
	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)processor_buf2 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(processor_buf2 + 4) = 0;


	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)markerfunc_buf1 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(markerfunc_buf1 + 4) = 0;

	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)markerfunc_buf2 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(markerfunc_buf2 + 4) = 0;

	
	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)xmit_buf1 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(xmit_buf1 + 4) = 0;

	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)xmit_buf2 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(xmit_buf2 + 4) = 0;


	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)rcv_buf1 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(rcv_buf1 + 4) = 0;

	//clear the nr_processor_u and merged_p_count at each time this function is called
	*(int *)rcv_buf2 = 0; // the first 4 byte(int is 4bytes long) used to put the
	// nr_processor_u, the second 4byte is used to put the merged_p_count
	*(int *)(rcv_buf2 + 4) = 0;
}
...
int init_module(void)
{
   ...
   memset(&mk_fun_head, 0, sizeof(struct mk_fun_data));
   INIT_LIST_HEAD(&mk_fun_head.list);
...
//	printk("device_read! transfer to user\n");
...
extern void POWER_TRIGGER_DAQ(int turnon);
...
*/
	switch (cmd) {
		case POWERMEMO_IOCTL_BEGINTEST:
			reinit_rec_buf();//to clear the buffer
			/* This function is located in kernel/driver/leds/Leds-gpio.c to trigger DAQ */
			POWER_TRIGGER_DAQ(1);
		...
		case POWERMEMO_IOCTL_ENDTEST:
			/* This function is located in kernel/driver/leds/Leds-gpio.c to stop DAQ */
			POWER_TRIGGER_DAQ(0);
			sync_flag = 0;
			powermemo_avail = 0;
#ifdef DEBUG_MIN
